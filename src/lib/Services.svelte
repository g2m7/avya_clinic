...
<section bind:this={section} class="py-24 px-4 bg-white" {id} use:scrollTo>
	<div class="container mx-auto">
		<div class="text-center mb-20 max-w-3xl mx-auto">
            <span class="text-[#BED173] font-medium tracking-widest uppercase text-sm mb-4 block">What We Treat</span>
		...
		<div bind:this={grid} class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
			{#each services as service}
				<div class="group relative bg-[#faf5f0] rounded-3xl overflow-hidden cursor-pointer h-[400px]">
                     <!-- Image -->
            ...
            <div class="group bg-[#527359] rounded-3xl overflow-hidden h-[400px] p-10 flex flex-col justify-center text-white relative">
                 <div class="absolute top-0 right-0 w-32 h-32 bg-[#BED173] rounded-full blur-3xl opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>

                 <h3 class="text-3xl font-light mb-6">Additional Services</h3>
                 <ul class="space-y-4">
                    {#each additional_services as item}
                        <li class="flex items-start">
                             <svg class="w-5 h-5 mr-3 mt-1 flex-shrink-0 text-[#BED173]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
